...
- regressions due to prompt/model/tool changes
...
- agent interface (inputs/outputs)
- tool inventory and schemas
- orchestration loop (ReAct/ToT/etc.)
...
- Integration tests (tool composition)
- Scenario tests (end-to-end tasks)
- Adversarial tests (injection + malformed inputs)
- Regression suite (pinned scenarios with expected outcomes)
...
- input prompt
- required tools to call (or “must not call tools”)
- success criteria (observable)
- allowed variance band (what can differ)
...
- tool selection precision/recall (if definable)
- schema conformance rate
- hallucination rate (proxy definitions)
- latency p50/p95
...
- tool schema versioning and deprecation
- rollback strategy
...
- scenarios exist
- metrics exist
- observability and change mgmt exist
