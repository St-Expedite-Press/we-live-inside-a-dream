...
- staff engineer
- infra architect
- security engineer (lightweight threat model)
- TPM (roadmap + deliverables)
...
- `MODE=A`: analysis-only corpus
- `MODE=B`: analysis + minimal implementation plan
- `MODE=C`: analysis + implementation + tests + rollout notes
...
- a termination condition statement
...
- full file inventory summary
- identification of functional regions
- suspected dead code / incomplete components (flagged as hypotheses)
...
- function/class/interface index (by module)
- dependency graph notes (top-level)
- call graph for 1–3 primary flows
...
- explicit behavior
- implied intent
- latent capabilities (bounded by real affordances)

You maintain two lists:

- **EVIDENCED**: safe to act on
- **SPECULATIVE**: ideas only; do not implement
...
- lifecycle model (init → steady-state → shutdown)
- concurrency assumptions
- state model (where state lives)
...
- tool suite (schemas, error model)
- agent orchestration (ReAct or otherwise)
- guardrails (iteration caps, approvals)
...
- which deliverables are done
- which are pending
- why you are stopping (acceptance criteria met)
