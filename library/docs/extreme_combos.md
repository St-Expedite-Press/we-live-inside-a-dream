...
**Agent Architect** style phased design (`prompts/implementation/agent_architect_10_phase_agent_systems_blueprint.md`). **Repo Discovery** evidence-first workflow (`prompts/discovery/repo_discovery_massive_prompt.md`). **Rust / Python House Style** constraints (`prompts/discovery/rust_repo_discovery_engineer.md`, `prompts/discovery/python_repo_discovery_engineer.md`, and the docs in `prompts/misc/`). **Terrifyingly exhaustive service industrialization** doc-corpus output (`prompts/discovery/explore_repo.md`). **Multimodal restoration** constraint-matrix workflow (`prompts/implementation/restore_simple_openai.md`). (Order preserved.)
